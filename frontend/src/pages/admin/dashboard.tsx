  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [usersRes, requestsRes, companiesRes, invoicesRes] =
          await Promise.all([
            api.admin.getUsers(),
        ...
        const newUsers = (usersRes.data || []).filter(
          (user: { createdAt: string }) => new Date(user.createdAt) > oneWeekAgo
        );
        ...
        setStats({
          ...
          activeRequests: (requestsRes.data || []).filter(
            (req: { status: string }) =>
              ["PENDING", "ACCEPTED", "IN_PROGRESS"].includes(req.status)
          ).length,
          totalCompanies: (companiesRes.data || []).length,
          totalVehicles: (companiesRes.data || []).reduce(
            (acc: number, company: { vehicles?: any[] }) =>
              acc + (company.vehicles?.length || 0),
            0
          ),
          totalUsers: (usersRes.data || []).length,
          totalRevenue: (invoicesRes.data || []).reduce(
            (acc: number, inv: { amount?: number }) => acc + (inv.amount || 0),
            0
          ),
          newUsersThisWeek: newUsers.length,
          revenueThisWeek: newInvoices.reduce(
            (acc: number, inv: { amount?: number }) => acc + (inv.amount || 0),
            0
          ),
        });
  ...
  return (
    <motion.div className="container mx-auto space-y-8 p-8">
      ...
      >
        ...
        {/* Replace the following cards with actual data similarly */}
        <motion.div variants={itemVariants}>
        ...
        <motion.div variants={itemVariants}>
          <Card className="border-l-4 border-l-purple-500">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +${stats?.revenueThisWeek.toFixed(2)} this week
              </p>
            </CardContent>
          </Card>
        </motion.div>
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Distribution of rescue requests by status
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {requestStats.map((item) => (
                <div key={item.status} className="space-y-1">
                  ...
                  <Progress
                    value={(item.count / (stats?.totalRequests || 1)) * 100}
                    className="h-2"
                  />
                </div>
              ))}
