...
import { Loader2, FileText, Wrench, Truck, DollarSign, Users, Building2, ShieldAlert } from "lucide-react"
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const newUsers = usersRes.data.filter((user: { createdAt: string }) => 
          new Date(user.createdAt) > oneWeekAgo
        )

        const newInvoices = invoicesRes.data.filter((inv: { createdAt: string }) => 
        ...
        setStats({
          ...
          activeRequests: requestsRes.data.filter(req => 
            ['PENDING', 'ACCEPTED', 'IN_PROGRESS'].includes(req.status)
          ).length,
          totalCompanies: companiesRes.data.length,
          totalVehicles: companiesRes.data.reduce((acc, company) => 
            acc + (company.vehicles?.length || 0), 0
          ),
          totalUsers: usersRes.data.length,
          totalRevenue: invoicesRes.data.reduce((acc, inv) => 
            acc + (inv.amount || 0), 0
          ),
          newUsersThisWeek: newUsers.length,
          revenueThisWeek: newInvoices.reduce((acc, inv) => 
            acc + (inv.amount || 0), 0
          )
        })
        ...
        const requestsByStatus = Object.entries(
          requestsRes.data.reduce((acc, req) => {
